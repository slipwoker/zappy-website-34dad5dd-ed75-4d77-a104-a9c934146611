document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getErrorContainer(field) {
      ...
      if (id) {
        const existing = document.querySelector('[data-error-for="' + id + '"]');
      ...
      if (id) errorEl.setAttribute('data-error-for', id);
    ...
    function validateField(field) {
      ...
      if (type === 'email' || field.dataset.validate === 'email') {
        if (!VALIDATORS.email(value)) {
          showError(field, ERROR_MESSAGES.email);
          return false;
        }
      }
      ...
      if (type === 'tel' || field.dataset.validate === 'tel') {
        if (!VALIDATORS.tel(value)) {
          showError(field, ERROR_MESSAGES.tel);
          return false;
        }
      }
      ...
      if (field.hasAttribute('minlength')) {
        ...
        if (!VALIDATORS.minlength(value, min)) {
          showError(field, ERROR_MESSAGES.minlength + ' (min ' + min + ' characters)');
          return false;
        }
      }
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '34dad5dd-ed75-4d77-a104-a9c934146611',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '34dad5dd-ed75-4d77-a104-a9c934146611';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '34dad5dd-ed75-4d77-a104-a9c934146611';
